...
const wait = (time = 500) => __awaiter(this, void 0, void 0, function* () { return new Promise((resolve) => setTimeout(resolve, time)); });
/////////////////////
///// QUAD TREE /////
/////////////////////
...
class QuadTreeNode {
    ...
    draw(ctx, x, y, size, lev, data, nbs) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (lev === data.length - 1) {
                ...
                for (let i = 0; i < NCHILDREN; i++) {
                    if (this.childrenBitmask[i]) {
                        yield data[lev + 1][this.children[i]].draw(ctx, x + (i % 2) * hs, y + Math.floor(i / 2) * hs, hs, lev + 1, data);
                    }
                }
            }
    ...
    drawDirect(ctx, lineWidth = 4, strokeStyle = 'rgba(150, 150, 250, 0.5)') {
        ...
        for (let lev = 0; lev < this.nLevels; lev++) {
            ...
            if (lev < nLevels - 1) {
                if (!n.childrenBitmask[childIndex]) {
                    ...
                    const hs = n._size / 2;
                    this.data[lev + 1].push(new QuadTreeNode(n._x + hs * (childIndex % 2), n._y + hs * Math.floor(childIndex / 2), hs, lev + 1));
                    parent = newParent;
                }
                else {
...
////////////////////////
class TravNode {
    constructor(parent, parentIdx, childIdx, level, data) {
        this.parent = parent;
    ...
    draw(stroke, fill) {
        ...
        // ctx.strokeStyle = "red";
        if (stroke)
            ctx.strokeRect(pos[0], pos[1], pos[2], pos[2]);
        if (fill)
            ctx.fillRect(pos[0], pos[1], pos[2], pos[2]);
    }
}
const getChildIndexForDirection = (dir, i) => {
    let childIdx = i * 2; // NX -> 0 or 2
    if (dir % 2 === 1)
        childIdx = i; // NY -> 0 or 1
    if (dir === 2)
        childIdx += 1; // PX -> 1 or 3
    if (dir === 3)
        childIdx += 2; // PY -> 2 or 3
    return childIdx % 4;
};
const getNeighbourGrtrEqSz = (tn, dir, data) => {
    if (tn.level === 0)
        return null;
...
const getNeighboursSmSz = (tn, tnNb, dir, neighbours, data) => {
    const candidates = [];
    if (tnNb.parent != null)
        candidates.push(tnNb);
...
const getNeighbours = (tn, neighbours, data) => {
